...
from utils import get_image_file_list, check_and_read

logger = get_logger()
# Get logger of the current run - time, date, and run ID

class TextRecognizer(object):
    #Argument is the directory to the inference result folder
    def __init__(self, args, logger=None):
        if os.path.exists(f"{args.rec_model_dir}/inference.yml"):
            ...
            #Load and check the inference.yml file
            model_name = model_config.get("Global", {}).get("model_name", "")

            #Load and check model name in the config file
            ...
            if args.rec_char_dict_path == "./ppocr/utils/ppocr_keys_v1.txt":
                '''
                If see char dict, simply get the file
        ...
        #Process through the algorithm and select the corresponding decoder
        postprocess_params = {
            "name": "CTCLabelDecode",
        ...
        #Take the module decoder from respective file
        self.postprocess_op = build.build_post_process(postprocess_params)
...
def main(use_gpu=None, image_dir=None, rec_model_dir=None):
    ...
    for ino in range(len(img_list)):
        ...
        return {'rec_text': rec_res[ino][0],
                'rec_score': rec_res[ino][1]}
    if args.benchmark:
